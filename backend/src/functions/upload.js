...
app.http('upload', {
  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      return { status: 200, headers: corsHeaders, body: JSON.stringify({ tekst, bestandsnaam: bestand.name, tekst_lengte: tekst.length }) };
    } catch (error) {
      context.log('Fout bij uploaden:', error.message);
      return { status: 500, headers: corsHeaders, body: JSON.stringify({ error: 'Fout bij verwerken bestand', details: error.message }) };
    }
  }
});
