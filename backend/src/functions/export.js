  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (!secties || !Array.isArray(secties)) {
        return { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'secties array is verplicht' }) };
      }
      const children = [];
      ...
      return { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'Fout bij genereren DOCX', details: error.message }) };
    }
  }
});
